...
export default function FileUploadForm() {
  ...
  const onSubmit = async (data: { file: File[]; name: string }) => {
    setIsUploading(true);
    setUploadProgress(0);
    ...
    try {
      ...
      const errorMessage = (axios.isAxiosError(error) && error.response?.data?.error) || 'Upload failed';
      setUploadResult({
        success: false,
  ...
  const Dropzone = ({ onDrop, maxSize, accept }: { onDrop: (acceptedFiles: File[]) => void; maxSize: number; accept: Record<string, string[]> }) => {
